(ns vura.holidays-edge-cases-test
  (:require
   [vura.core :as v]
   [clojure.test :refer [deftest is testing]]
   [vura.holidays.compiler :as c]
   [vura.holidays.util :refer [parse-definition]]))
...
(deftest nth-weekday-edge-cases
  ...
  (testing "4th Thursday in November - Thanksgiving edge cases"
    (let [thanksgiving (c/compile-in-month
      ...
      (is (nil? (thanksgiving (->day-time-context 2019 11 28)))
          "Nov 28, 2019 (5th Thursday) should NOT be Thanksgiving"))))
...
(deftest parse-definition-edge-cases
  (testing "Complex parsing edge cases"
    (is (= (parse-definition "orthodox easter -47 and if sunday then next monday")
           {:orthodox? true :easter? true :offset -47 :and? true
