...
(deftest holiday-definition-parsing
  ...
  (is (= (c/parse-definition "orthodox -47")
         {:orthodox? true, :offset -47}))
  (is (= (c/parse-definition "orthodox and if sunday then next monday")
         {:orthodox? true, :offset 0, :and? true, :statements ['("sunday" "then" "next" "monday")]}))
  (is (= (c/parse-definition "easter")
         {:easter? true, :offset 0}))
  (is (= (c/parse-definition "easter 26")
         {:easter? true, :offset 26}))
  (is (= (c/parse-definition "easter -2 prior to 2019")
         {:easter? true, :offset -2, :unknown ["prior" "to" "2019"]}))
  (is (= (c/parse-definition "easter -6 P7D")
         {:easter? true, :offset -6, :unknown ["p7d"]}))
  (is (= (c/parse-definition "julian 06-15")
         {:julian? true, :day-in-month 15, :month 6}))
  (is (= (c/parse-definition "julian 12-25 and if saturday, sunday then next monday")
