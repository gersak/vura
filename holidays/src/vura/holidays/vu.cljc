;; This file is autogenerated using vura.holidays.compile/-main

(ns vura.holidays.vu
  (:require 
  [clojure.string]
  [vura.holidays :refer [is-holiday?]]
  [vura.holidays.name :as n]
  [vura.holidays.compiler :as compiler]))


(def holidays
  {"02-22" {:name (partial n/get-name "Public Holiday")},
 "08-15" {:name (partial n/get-name "08-15")},
 "01-01" {:name (partial n/get-name "01-01")},
 "easter 39" {:name (partial n/get-name "easter 39")},
 "10-29" {:name "Jour de l'Unité"},
 "07-24" {:name "Journée Mondiale des Enfants"},
 "12-27" {:name (partial n/get-name "Public Holiday")},
 "10-05" {:name (partial n/get-name "Constitution Day")},
 "easter -2" {:name (partial n/get-name "easter -2")},
 "08-16" {:name (partial n/get-name "Public Holiday")},
 "12-26" {:name "Fête de Famille"},
 "easter 1" {:name (partial n/get-name "easter 1")},
 "07-30" {:name (partial n/get-name "Independence Day")},
 "03-05" {:name "Fête des chefs coutumiers"},
 "05-01" {:name (partial n/get-name "05-01")},
 "02-21"
 {:name "Mémoire du père de l'indépendance Rév. Dr. W.H. Lini"},
 "12-25" {:name (partial n/get-name "12-25")}}
)


(def locale-holiday-mapping
  (reduce-kv
    (fn [result definition name-mapping]
      (assoc result
             (compiler/compile-type (compiler/parse-definition definition))
             name-mapping))
    nil
    holidays))


(defn holiday?
  [context]
  (some
   (fn [[pred naming]]
     (when (pred context)
       naming))
   locale-holiday-mapping))


(defmethod is-holiday? :vu
  [_ context]
  (holiday? context))